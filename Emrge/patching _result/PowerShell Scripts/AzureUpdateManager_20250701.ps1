...
$Get_Subscription = @()
$Get_Subscription_vm = @()
$Get_Subscription_sqlserver = @()
$vm_tags = @()
$sql_tags = @()
$tenant_subs = @()

# 1 # Start for querying patching result 
...
# 2 # Virtual Machine, SQL server details
...
#    $tenant = Get-AzTenant -TenantId $tenant_id 
    ...
    # Collect SQL server details, add SQL server data to the array
    foreach ($sqlserver in $sqlservers) {
        $sql_databases = Get-Azsqldatabase -servername $sqlserver.ServerName -resourcegroupname $sqlserver.ResourceGroupName
        foreach ($sqldatabase in $sql_databases) {
            $sql_tags += [PSCustomObject]@{
                SubscriptionName = $sub.Name
...
# 3 $ Export result: Setup destination storage account
...
$jsonBlobName_sqlserver = "azure_sql_server.json"  
...
# Generate SQL server result
$tempFile_sqlserver = [System.IO.Path]::GetTempFileName()
$sqlserver_tags_json = $sql_tags | ConvertTo-Json -Depth 10
$sqlserver_tags_json | Out-File -FilePath $tempFile_sqlserver -Encoding utf8
...
# Export SQL server result to storage account
Set-AzStorageBlobContent -File $tempFile_sqlserver -Container $containerName -Blob $jsonBlobName_sqlserver -Context $context -Force
